...
export async function appendToOverviewSheet(customerData, docUrl) {
  ...
  try {
    ...
    const values = [
      [timestamp, customerData.name, customerData.email, customerData.company, customerData.message, docUrl]
    ];

    await sheets.spreadsheets.values.append({
      spreadsheetId,
      range: 'Sheet1!A:F', // Modify 'Sheet1' if your tab is named differently
